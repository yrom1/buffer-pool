...
class LRUBufferPool:
    ...
    def read(self, block_id):
        if block_id in self.buffer_pool:
            ...
            self.buffer_pool[block_id] = (data, dirty_bit)  # Move to the end (most recently used)
            return data
        else:
    ...
    def write(self, block_id, data):
        if block_id in self.buffer_pool:
            ...
            if len(self.buffer_pool) >= self.capacity:
                oldest_block_id, (oldest_data, oldest_dirty_bit) = self.buffer_pool.popitem(last=False)
                if oldest_dirty_bit:
    ...
    def read_from_disk(self, block_id):
        offset = block_id * (self.block_size + 5)  # Calculate the offset based on block ID
    ...
    def write_to_disk(self, block_id, data):
        ...
        with open(self.file_name, 'rb+') as file:
            file.seek(offset)
            header = struct.pack('<IB', block_id, 1)  # Set dirty bit to 1 when writing to disk
            file.write(header)
            file.write(data)
